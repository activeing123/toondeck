...
export const DICT: Record<string, { en: string; zh: string }> = {
  ...
  "agents.modelDefault": { en: "agent's own default", zh: "agent 自带默认" },
  "agents.customModel": { en: "＋ custom model name…", zh: "＋ 自定义模型名…" },
  "agents.customModelPrompt": { en: "type the exact model name:", zh: "输入准确的模型名：" },
  ...
  "agents.nameLabel": { en: "Name (just for you — any label works)", zh: "名称（随便起，只在本机显示）" },
  "agents.urlLabel": { en: "Base URL (the gateway address, usually ends with /v1)", zh: "接口地址（网关网址，一般以 /v1 结尾，从中转站或官网复制）" },
  "agents.keyLabel": { en: "API key (the pass for using the model)", zh: "API Key（模型的通行证）" },
